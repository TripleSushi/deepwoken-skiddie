...
local hashMap = {}
local sha256KTable = nil
local remoteNode = nil
local sha256 = nil
...
local function initSHA256()
    ...
    sha256 = coroutine.wrap(function(msg)
        while true do
            ...
            msg = coroutine.yield(
                number2String(H[1], 4) .. number2String(H[2], 4) ..
                number2String(H[3], 4) .. number2String(H[4], 4) ..
                number2String(H[5], 4) .. number2String(H[6], 4) ..
                number2String(H[7], 4) .. number2String(H[8], 4)
            )
        end
    end)
end
...
function KeyHandler.init()
    ...
    warn("KeyHandler initialized successfully")
    return true
end

function KeyHandler.getKey(name)
    if not sha256 then
        warn("KeyHandler not initialized. Call KeyHandler.init() first")
        return nil
    end
...
function KeyHandler.clearCache()
    hashMap = {}
end

function KeyHandler.exists(name)
    return KeyHandler.getKey(name) ~= nil
end
