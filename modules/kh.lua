...
KeyHandling.init = LPH_NO_VIRTUALIZE(function()
	...
	while true do
        ...
        warn(string.format("KeyHandler retry (%i attempts) with results (%s, %s)", retries, tostring(remoteTable), tostring(randomTable)))

		retries = retries + 1

		if retries >= 10 then
			-- Warn.
			warn("KeyHandler failed to initialize after 10 attempts.")
